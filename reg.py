...
def rg():
    if fname.get()=="" or lname.get()=="" or uname.get()=="" or age.get()=="" or pno.get()=="" or secans.get()=="" or pas.get()=="" or conpas.get()=="" or secsel.get()=="select" : 
            l=Label(register,text="All fields are required",font=('times new roman',15),fg='red')
            l.place(x=300,y=500)
    elif pas.get()!=conpas.get():
            l=Label(register,text="Passwords does not match",font=('times new roman',15),fg='red')
            l.place(x=300,y=500)
    else:
        Firstname=fname.get()
        ...
        conn= mysql.connector.connect(host="localhost",user="root",port="3307",password="1234",database="users_db")
        my_cursor=conn.cursor()
        query=("select * from my_database where Username=%s")
        value=(uname.get(),)
        ...
        if row!=None:
            ...
            q1="insert into my_database(Firstname,Lastname,Username,Age,Phone_number,Security_Question,Security_Answer,Password,Confirm_Password) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
#bg=ImageTk.PhotoImage(file=r"C:\Users\nikhi\OneDrive\Pictures\41674872-white-concrete-wall-with-creeper-plants-for-background.jpg")
#lbl_bg=Label(register,image=bg)
#lbl_bg.place(x=0,y=0,relheight=1,relwidth=1)
label=Label(register,text="SIGN IN",font=('times new roman',45,'bold'),fg='green')
label.pack()
...
conpas=Entry(register,font=('times new roman',18) )
conpas.place(x=710,y=450)
regbtn=Button(register,text="SIGN IN",font=('times new roman',30,'bold'),fg='green',command=rg)
regbtn.place(x=300,y=550,relwidth=0.20,relheight=0.1)
logbtn=Button(register,text="Login",font=('times new roman',30,'bold'),fg='green',command=log_return)
logbtn.place(x=670,y=550,relwidth=0.20,relheight=0.1)
